<style>
@import "@/../public/static/css/common.css?v=sdsderfrgfgdfdf";
@import "@/../public/static/css/icono.min.css";
  ...
  <div class="visitorBody">
    <div id="app" class="chatCenter">
        ...
        <div class="chatBoxSend">
          ...
          <div class="visitorIconBox">
            <el-tooltip content="发送表情" placement="top">
              <div class="iconBtn iconfont icon-xiaolian" style="margin-left:10px;font-size: 24px;cursor: pointer;"
          ...
          <el-input show-word-limit maxlength="100" :rows="2" type="textarea" resize="none" class="visitorEditorArea"
                    @focus="scrollBottom, showIconBtns=false" @blur="scrollBottom, showIconBtns=false"
                    v-model="messageContent" v-on:keyup.enter.native="chatToUser">
          </el-input>
          <el-button type="primary" size="mini" class="visitorEditorBtn" :disabled="sendDisabled||messageContent==''"
                     v-on:click="chatToUser();showIconBtns=false">{{ flyLang.sent }}
          </el-button>
        </div>
      </div>
      <div class="chatArticle">
        ...
        <div class="allNotice">{{ kefuInfo.allNotice }}</div>
      </div>
      <!--//创建音频元素-->
...
import {
  ...
  notify,
  flashTitle,
} from "@/../public/static/js/functions";
  ...
  methods: {
    ...
    OnMessage: function (e) {
      ...
      if (redata.type == "kfOnline") {
        ...
        this.showTitle(msg.name + "," + this.Service_LANG[this.LANG]['chating']);
        this.scrollBottom();
      ...
      if (redata.type == "close") {
        this.chatTitle = this.Service_LANG[this.LANG]['closemes'];
        $(".chatBox").append("<div class=\"chatTime\">" + this.chatTitle + "</div>");
        this.scrollBottom();
        this.socket.close();
      ...
      if (redata.type == "force_close") {
        this.chatTitle = this.Service_LANG[this.LANG]['forceclosemes'];
        $(".chatBox").append("<div class=\"chatTime\">" + this.chatTitle + "</div>");
        this.scrollBottom();
        this.socket.close();
        this.socketClosed = true;
      ...
      if (redata.type == "auto_close") {
        this.chatTitle = this.Service_LANG[this.LANG]['autoclosemes'];
        $(".chatBox").append("<div class=\"chatTime\">" + this.chatTitle + "</div>");
        this.scrollBottom();
        this.socket.close();
        this.socketClosed = true;
    ...
    getNotice: function () {
      let _this = this;
      $.get("/notice?kefu_id=" + this.KEFU_ID, function (res) {
        var code = res.code;
        if (code != 200) return;
    ...
    init: function () {
      ...
      this.LANG = checkLang()
      this.chatTitle = this.Service_LANG[this.LANG]['connecting']
      this.flyLang = this.Service_LANG[this.LANG]
    },
    //表情点击事件
